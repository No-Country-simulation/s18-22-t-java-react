    ...
    <>
      ...
      <div className="flex flex-col text-[#1A2C33] w-[420px] h-[360px] shadow-2xl p-7 rounded-xl">
        <div className="flex flex-col text-start">
          <p className="text-2xl font-medium">{waitList.name}</p>
          <p className="text-[#3C4C51] font-medium mb-4">{waitList.specialization}</p>
          <p className="text-[#3C4C51]">{waitList.date}</p>
          <p className="text-[#3C4C51] mb-6">{waitList.establishment}</p>
        </div>
        {confirmWaitList ? (
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-3">
              {Array.from({ length: 5 }).map((_item, index) => ((
                <div key={index}>
                  {waitList.position && waitList.position > index ? (
                    <IconUser size={36} />
                  ) : (
            ...
            <span className="text-[#3C4C51] mt-2">Estás {waitList.position}° de la lista de espera</span>
