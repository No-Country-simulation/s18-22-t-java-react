...
export function DoctorCard({ id_appointment, id_doctor, name, specialty, place = "Clínica Colón", img, dashboard, date, startTime }: Props) {
  ...
  const cancelAlert = () => {
    const alert = {
      ...
      description: <p>Querrías cancelra tu cita asignada para el <span className="font-bold">{formattedDate}</span> a las <span className="font-bold">{startTime.replace(":00", "")}</span> en <span className="font-bold">{place}</span>?</p>,
  ...
  const updateAppointment = () => {
    ...
    cancelAppointment(id_appointment)
  }
